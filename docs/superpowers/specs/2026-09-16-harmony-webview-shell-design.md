...
**范围订正（重要）**：原生移植路线下的「`packages/` 零改动」约束**不再原样适用**。
那条约束的目的是保证移植不去篡改被参照的实现；本方案里 web **就是产品本身**，而项目
负责人的原话是「用现有的 web 前端包装一个 app，**然后页面兼容下鸿蒙**」—— 让页面适配
鸿蒙本就在范围内。

因此：
- **`packages/web` 允许为 ArkWeb 兼容性做修改**，前提是不得让桌面/移动浏览器上的行为
  变差；这类修改应当是渐进增强或纯修复，不是分叉。
- **`packages/server` 仍然零改动。** 服务端没有任何理由为了一个客户端外壳而变。
...
- 校验方式：`GET ${baseUrl}/` 并确认返回的 HTML 含 `CC Pet` 标题。
  **不要用 `/api/health`** —— 该路由并非免鉴权：`authGuard` 是全局 `onRequest` 钩子，
  无 token 时回 401，且**连续 5 次失败会把来源 IP 锁 5 分钟**。按原方案做，用户输错一次
  地址就可能把自己锁在门外。（本条为设计时的错误假设，实现时实测发现。）
...
**本节原先的结论是错的，已在实现阶段推翻，保留订正记录。**

原文断言「WebView 内容对 `uitest` 不可见、坐标点击无法聚焦 web 输入框」，依据是 spike
阶段的观察。实测结果相反：`uitest` 能取到 WebView 内部的完整无障碍树（含坐标），坐标点击
能聚焦 web `<input>` 并唤起输入法，`uitest uiInput text` 原样提交。spike 阶段点不动，
几乎可以确定是从缩放截图反推坐标所致，而非平台限制。

因此实际的验证结构是：

- `packages/web` 自身的 339 个测试覆盖 web 行为
- `harmony/scripts/device-shell-probe.sh` 端到端覆盖「壳子没把 web 弄坏」——清数据 →
  原生地址页 → WebView 内登录 → 消息往返
- 原生部分（返回键、错误页、地址录入）本就可断言

**教训**：一次「做不到」的观察，在成为设计约束之前必须再验一次。它比「做得到」的观察更
危险——前者会让人停止尝试。
...
- 附件选择能唤起系统文件选择器
- 加载失败时有可见的重试与换地址入口
- `packages/server` 零改动；`packages/web` 的任何改动必须在桌面浏览器上回归通过
